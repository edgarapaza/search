...
if($proyecto > 0){
  ...
  */
  ...
  echo "<section id='sectionOtorgantes'>";
      echo "Lista de Otorgantes Juridicos";
          $sql_codOtorgantes      = "SELECT cod_inv_ju FROM escriotor1 WHERE cod_sct = $codigo_escritura;";
          ...
          while($lista_Otorgantes = $result_codOtorgantes->fetch_array()){
            ...
            $sql_persona    = "SELECT Raz_inv FROM involjuridicas1 WHERE Cod_inv = ".$lista_Otorgantes["cod_inv_ju"];
    ...
    echo "<section id='sectionFavorecidos'>";
        echo "Lista de favorecidos Juridicos";
        $sql_codFavorecidos      = "SELECT cod_inv_ju FROM escrifavor1 WHERE cod_sct = $codigo_escritura;";
        ...
        while($lista_Favorecidos = $result_codFavorecidos->fetch_array()){
          ...
          $sql_persona    = "SELECT Raz_inv FROM involjuridicas1 WHERE Cod_inv = ".$lista_Favorecidos["cod_inv_ju"];
          $result_persona = $mysqli->query($sql_persona);
          $persona        = $result_persona->fetch_array();
          echo "<div class='alert alert-dismissable alert-info'>
          <strong>".$persona[0]."</div>";
...
else
{
  ...
  echo "<section id='sectionOtorgantes'>";
    echo "Lista de Otorgantes Juridicos";
        $sql_codOtorgantes      = "SELECT cod_inv_ju FROM escriotor1 WHERE cod_sct =".$dato2["cod_sct"];
        $result_codOtorgantes   = $mysqli->query($sql_codOtorgantes);
    ...
    echo "<section id='sectionFavorecidos'>";
        echo "Lista de favorecidos Juridicos";
        $sql_codFavorecidos      = "SELECT cod_inv_ju FROM escrifavor1 WHERE cod_sct =".$dato2["cod_sct"]."";
        $result_codFavorecidos   = $mysqli->query($sql_codFavorecidos);
        while($lista_Favorecidos = $result_codFavorecidos->fetch_array()){
          //echo "<br>".$lista_Favorecidos[0];
          $sql_persona    = "SELECT Raz_inv FROM involjuridicas1 WHERE Cod_inv = ".$lista_Favorecidos["cod_inv_ju"];
