...
<div class="container">
	<div class="row">
		<div class="col-md-12">
		    ...
		    <table class="table table-striped">
				...
				<tbody>
					<?php
					$new_sql = "SELECT cod_sct FROM escriotor WHERE cod_inv  = ".$codigo_involucrado;
					...
					if ($result_otorgantes = $mysqli->query($new_sql))
					{
						...
						while ($fila = $result_otorgantes->fetch_assoc()) {
							...
							$query1 = "SELECT cod_sct,num_sct, fec_doc, cod_sub, cod_pro FROM escrituras WHERE cod_sct = ".$fila["cod_sct"];
	...
	**********************************************************************************************
	...
	<div class="row">
		<div class="col-md-12">
			<h2 class="bg-info">Zona Favorecidos</h2>
    		<table class="table table-striped">
				<thead>
					<tr>
						<th>Num</th>
				...
				<tbody>
					...
					if ($result_favorecidos = $mysqli->query("SELECT cod_sct FROM escrifavor WHERE cod_inv  =  $codigo_involucrado;")) {
						echo "Numero: ".$result_favorecidos->num_rows;
						echo "<br> A favor de: ". $res_invol["nombre"];
						$i = 1;
						...
						if ($result_favorecidos->num_rows > 0) {
							while ($filaf = $result_favorecidos->fetch_assoc()) {
								...
								$query_f = "SELECT cod_sct,num_sct, fec_doc, cod_sub, cod_pro FROM escrituras WHERE cod_sct = ".$filaf["cod_sct"];
